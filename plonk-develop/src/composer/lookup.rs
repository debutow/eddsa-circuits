...
pub struct Table<F: Field> {
    ...
    columns: Vec<Vec<F>>, //表的基本内容（外层是列（异或有3列，l，r，值），内层是每一列的长度（表的长度，值是F域上的））
    lookups: Vec<usize>, //存查询 表中索引

    key_map: Map<Vec<F>, usize>, //key（异或就是【l，r】）， 对应表中索引
}

impl<F: Field> Table<F> {
    fn get_value_by_key(&mut self, key: &[F]) -> Result<Vec<F>, Error> {
        //key要符合设定的宽度。返回的key也可能是多个值
...
impl<F: Field> Table<F> {
    pub fn xor_table(bits: usize) -> Self {
        ...
        let size = 1 << bits * 2;
        let width = 3;
        let key_width = 2; //key只有l r，没有异或值
...
impl<F: Field> Composer<F> {
    ...
    pub(super) fn compute_sorted_values(&self) -> Vec<Vec<F>> {
        ...
        for (i, table) in self.tables.iter().enumerate() {
            let mut lookups: Vec<_> = (0..table.size).collect(); //表的索引一定是顺序（相当于先复制一遍表）
            lookups.extend(&table.lookups); //加入lookup
            lookups.sort(); //排序
