...
pub struct Prover<F: Field, D: Domain<F>> {
    domain_values: Map<String, Vec<F>>, //待插值的向量
    coset_values: Map<String, Vec<F>>, //coset fft后的值
...
impl<'a, F: Field, D: Domain<F>> Prover<F, D> {
    ...
    pub fn evaluate(&mut self, poly_label: &str, point_label: &str) -> Result<F, Error> {
        let label = format!("{}_{}", poly_label, point_label);  //自动组装新label
...
impl<'a, F: Field, D: Domain<F>> Prover<F, D> {
    ...
    pub fn prove<R: RngCore>(&mut self, cs: &mut Composer<F>, rng: &mut R) -> Result<(), Error> {
        ...
        self.add_challenge("zeta_omega", zeta * self.domain.generator()); //这个也算作challenge吗
        let (r, r_complement) = {
