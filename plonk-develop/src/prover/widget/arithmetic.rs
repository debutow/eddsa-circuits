...
mod tests {
    ...
    fn prover_arithmetic() -> Result<(), Error> {
        ...
        for public_i in &public_input {
            transcript.update_with_fr(public_i);
        }
        let my_alpha: Fr = transcript.generate_challenge();
        println!("testalpha {}", my_alpha);


        print!("compute quotient...");
        // prover.add_challenge("alpha", Fr::rand(rng));
        prover.add_challenge("alpha", my_alpha);
